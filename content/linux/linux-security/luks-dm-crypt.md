...
│         (복호화된 가상 블록 디바이스)                     │
├────────────────────────────────────────────────────────┤
│              dm-crypt (커널 device mapper)              │
│         암호화/복호화 엔진 (AES-XTS, ChaCha20...)        │
├────────────────────────────────────────────────────────┤
│              LUKS 헤더 (디스크 앞부분)                   │
│   매직넘버 | UUID | 암호 파라미터 | 키슬롯(0~31)          │
├────────────────────────────────────────────────────────┤
│          물리 블록 디바이스 (/dev/sda, /dev/nvme0n1p2)   │
└────────────────────────────────────────────────────────┘
```
...
LUKS 선택 ──────────────────────────────────────────┐
  일반 서버/노트북/스토리지                           │
  다중 패스프레이즈(키슬롯 여러 개) 필요             │
  헤더 손상 복구 필요                                │
  TPM2/FIDO2 바인딩 필요                            │
  표준 도구(cryptsetup) 생태계 활용                  │
                                                    │
Plain dm-crypt 선택 ─────────────────────────────────┤
  헤더 존재 자체를 숨겨야 하는 경우 (Plausible Deniability) │
  임베디드/특수 환경                                │
