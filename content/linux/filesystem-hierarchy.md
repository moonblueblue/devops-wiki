...
Filesystem Hierarchy Standard(FHS)는 리눅스/유닉스 계열 OS의 디렉토리 구조 표준이다.
FHS 3.0이 최신 버전이며, 2025년 11월부터 FreeDesktop.org가 관리한다.
...
누구나 읽고 쓸 수 있는 임시 디렉토리.
tmpfs로 마운트된 경우 재부팅 시 초기화되며,
`systemd-tmpfiles-clean.timer`가 주기적으로도 정리한다.
...
# 커널 파라미터 변경 — 비권장: 재부팅 시 초기화됨
echo 1 > /proc/sys/net/ipv4/ip_forward
# 영구 적용: /etc/sysctl.d/99-k8s.conf 에 설정 후 sysctl -p
```
...
/run/containerd/containerd.sock  # containerd 런타임 소켓
```
...
| Ubuntu 23.04+ | 완료 |
| RHEL 9 | 완료 |
| Alpine 3.23 | opt-in 지원 (기본 적용은 3.26~3.27로 연기) |
| Gentoo | 진행 중 |

### 왜 하는가

- **systemd 255** 에서 split-usr(부팅 시 /usr 별도 마운트) 지원 제거
  → 사실상 /usr merge가 필요해짐
...
| `/var/lib/containerd` | 컨테이너 데이터 격리 (containerd 환경) |
| `/var/lib/kubelet` | kubelet 데이터 격리 (K8s 노드) |
...
- [FHS 최신 공식 문서](https://specifications.freedesktop.org/fhs/latest/)
  (2025년 11월부터 FreeDesktop.org 관리)
