...
# 버전 확인 (0.23+ 권장, 2026-04 기준 최신 0.25.x)
bpftrace --version
```
      ...
      - name: monitor
        ...
        securityContext:
          ...
          capabilities:
            ...
            - CAP_SYS_PTRACE # /proc/<pid>/maps 접근,
                             # uprobe 대상 바이너리의
                             # 심볼 resolving에 필요
          # runAsNonRoot: true  # 주의: CAP_SYS_PTRACE는
          #   다른 프로세스 메모리 접근 권한이므로 non-root
          #   사용자가 소유해도 권한 상승 위험이 있다.
          #   심볼 해석이 필수가 아니면 CAP_SYS_PTRACE를
          #   제외하고 사전 추출된 심볼을 사용한다.
...
### BPF Token (Linux 6.9+) — 컨테이너 권한 위임

Linux 6.9에서 도입된 **BPF Token**은 "특권 프로세스가 비특권
프로세스에 정밀하게 범위가 제한된 BPF 권한을 위임"하는
메커니즘이다. 컨테이너 환경에서 `CAP_BPF`/`CAP_SYS_ADMIN`을
직접 부여하지 않고도 eBPF를 쓸 수 있다.

```mermaid
flowchart LR
    A["호스트 특권 데몬"] -->|"bpffs mount"| B["bpffs"]
    B -->|"token fd"| C["컨테이너 비특권 프로세스"]
    C -->|"bpf() with token"| D["제한된 BPF ops"]
```

| 기존 방식 | BPF Token 방식 |
|----------|--------------|
| `CAP_BPF` + `CAP_PERFMON`을 컨테이너에 부여 | 호스트가 bpffs로 토큰 발급 |
| 컨테이너가 모든 BPF 작업 가능 | 토큰에 담긴 범위만 가능 |
| 권한 상승 위험 | user namespace + BPF subsystem 범위 제한 |

```bash
# 호스트에서 bpffs 마운트 (위임 범위 지정)
mount -t bpf -o delegate_cmds=prog_load,map_create,\
delegate_progs=kprobe,delegate_maps=hash,array \
    bpf /sys/fs/bpf/delegated

# 컨테이너는 해당 bpffs를 볼륨으로 마운트
# libbpf 0.1+ 가 토큰을 자동 인식해 bpf() 호출에 전달
```

> **활용 예**: Kubernetes에서 Tetragon/Cilium Agent를
> `privileged: false`로 실행하고, node-level BPF daemon이
> 토큰을 발급하여 필요한 프로그램 타입만 허용하는 패턴이
> 확산되는 중이다. libbpf 및 주요 배포판(FOSDEM 2026 발표)
> 에서 점진적으로 지원이 확대되고 있다.
...
eBPF 프로그램은 일반 코드처럼 디버거를 붙일 수 없다. 대신
verifier 에러 해석, 커널 내부 출력, 바이트코드 덤프 세 가지
방법을 조합한다.

### 9.1 Verifier 에러 해석

```
libbpf: prog 'vfs_read_enter': BPF program load failed:
        Permission denied
libbpf: prog 'vfs_read_enter': -- BEGIN PROG LOAD LOG --
...
| 에러 메시지 | 원인 | 해결 |
|-----------|------|------|
| `invalid mem access` | null 체크 없이 포인터 역참조 | `if (ptr == NULL) return 0;` |
| `R0 !read_ok` | 초기화 안 된 레지스터 사용 | 변수 명시 초기화 |
| `unbounded loop` | verifier가 종료 증명 불가 | `#pragma unroll` 또는 `bpf_loop()` |
| `stack >= 512` | 스택 512B 초과 | Map으로 데이터 이동 |
| `too many instructions` | 1M 명령어 초과 | 함수 분리 (`BPF_PROG`) |

### 9.2 `bpf_printk` + `trace_pipe`

커널 내에서 디버그 출력을 찍는 표준 방법.

```c
// BPF C 코드
bpf_printk("vfs_read pid=%d count=%d\n", pid, count);
```

```bash
# 출력 확인 (모든 BPF 프로그램 공유 버퍼)
sudo cat /sys/kernel/debug/tracing/trace_pipe
```

> `bpf_printk`는 전역 링 버퍼를 공유하므로 프로덕션에서는
> 쓰지 않는다. 개발·디버깅 전용이며, 고빈도 경로에 넣으면
> 오버헤드와 출력 손실이 크다.

### 9.3 `bpftool`로 바이트코드 덤프

```bash
# 로드된 프로그램 목록
sudo bpftool prog list

# 특정 프로그램 xlated(verifier 변환 후) 바이트코드
sudo bpftool prog dump xlated id 42

# JITed 네이티브 코드 (CPU 아키텍처별)
...
| 용도 | 명령 |
|------|------|
| Verifier가 어떻게 코드를 재작성했는지 | `dump xlated` |
| 실제 CPU 명령어 확인 (성능 분석) | `dump jited` |
| Map에 데이터가 제대로 들어가는가 | `map dump` |
| 프로그램 JIT 활성화 여부 | `prog show` (`jited: yes`) |
