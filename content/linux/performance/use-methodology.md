...
리소스 목록: CPU, Memory, Disk, Network, Controller, Interconnect
...
| 방법론 | 질문 | 적합한 상황 |
|--------|-----|-----------|
| USE | "서버가 왜 느린가?" | 인프라 리소스 병목, 하드웨어/OS 수준 트러블슈팅 |
| RED | "API 응답이 왜 느린가?" | 마이크로서비스 성능 모니터링, SLO/SLI 알림 설계 |
| 함께 사용 | 두 질문 모두 | RED로 사용자 영향 감지 후 USE로 인프라 근본 원인 파악 |
...
```mermaid
flowchart TD
    ALERT([알림 수신]) --> QUICK[60초 체크리스트]

    QUICK --> HOTSPOT{핫스팟?}

    HOTSPOT -- CPU 이상 --> CPU_U{CPU Util > 80%?}
    HOTSPOT -- 메모리 이상 --> MEM_S{Swap 활성화?}
    HOTSPOT -- I/O 이상 --> DISK_U{%util > 80%?}
    HOTSPOT -- 없음 --> APP[RED 방법론 적용]
```
...
```mermaid
flowchart TD
    CPU_U{CPU Util > 80%} -->|예| CPU_SAT{런큐 > nproc?}
    CPU_U -->|아니오| CPU_ERR[MCE 확인]
    CPU_SAT -->|예| CPU_FIX[스케일업]
    CPU_SAT -->|아니오| PROFILE[프로파일링]
```
...
```mermaid
flowchart TD
    MEM_S{Swap 활성?} -->|예| MEM_WHO[메모리 과다 프로세스]
    MEM_S -->|아니오| MEM_PSI[PSI 확인]

    DISK_U{%util > 80%} -->|예| DISK_SAT{aqu-sz > 1?}
    DISK_U -->|아니오| DISK_ERR[SMART 확인]
    DISK_SAT -->|예| DISK_WHO[I/O 프로세스 특정]
    DISK_SAT -->|아니오| DISK_WAIT[랜덤 I/O 확인]
```

**60초 체크리스트**: `uptime` → `dmesg` → `vmstat` → `top`
