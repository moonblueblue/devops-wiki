...
graph TD
    ...
    subgraph kif["커널 인터페이스"]
        IPT_K["iptables (x_tables)"]
        NFT["nftables"]
    end
    subgraph netfilter["커널 프레임워크: netfilter"]
        NF["훅(hook) 포인트 → 패킷 처리 체인 연결"]
    end
    KNET["Linux Kernel Network Stack"]
...
```mermaid
graph TD
    FAM["패밀리 (family)<br/>ip (IPv4) · ip6 (IPv6) · inet (IPv4+IPv6)<br/>arp · bridge · netdev"]
    --> TBL["테이블 (table): 임의 이름"]
    --> CHN["체인 (chain): 훅 포인트 연결"]
    --> RULE["규칙 (rule): 매칭 + 액션"]
```
...
graph TD
    NIC["인터페이스"]
    --> ZONE["존 선택<br/>(기본: public)"]
    ZONE -->|"인터페이스에 존 할당"| RULES["존별 규칙 세트 적용"]
```
...
graph TD
    CLI["클라이언트"]
    -->|"ClusterIP 접근"| PRE2["PREROUTING (nat)<br/>kube-proxy가 생성한 DNAT 규칙"]
    --> POD_IP["실제 Pod IP:Port"]
```
...
graph LR
    subgraph trad["전통적 경로"]
        direction LR
        PKT1["패킷"] --> IPT2["iptables/netfilter"] --> DNAT["kube-proxy DNAT"] --> POD1["Pod"]
    end
    subgraph cilium["Cilium 경로"]
        direction LR
        PKT2["패킷"] --> EBPF["eBPF (XDP/TC hook)<br/>커널 네트워크 스택 우회"] --> POD2["Pod"]
    end
```
