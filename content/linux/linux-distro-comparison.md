...
| 항목 | 내용 |
|------|------|
| 최신 LTS | **26.04 LTS** "Resolute Raccoon" (2026년 4월 23일 출시 예정) |
...
Fedora ──→ CentOS Stream ──→ RHEL
  (upstream)   (midstream)     (downstream)
                                    │
                                    ├──→ Rocky Linux (소스 기반, 1:1 호환)
                                    └──→ AlmaLinux (소스 기반, ABI 호환)
```
...
| **Rocky Linux** | 10.1 (2025.11 GA), 10.2 예정 (2026.05) | RHEL 10과 동일 주기 |
...
> ⚠️ musl의 DNS 리졸버는 단일 스레드로 동작한다.
> 고빈도 DNS 조회 워크로드(서비스 메시, 대규모 마이크로서비스 등)에서
> 병목이 발생할 수 있다. 이 경우 `ndots` 값 조정이나
> `resolv.conf` 튜닝이 필요하다.
...
| 패키지 매니저 | apt (dpkg 기반) |
| 지원 기간 | 총 5년 (Debian 지원 3년 + LTS 2년, Debian 13은 ~2030년까지) |
...
|--------|------|-----------|-----------|------|
| **Amazon Linux 2023** | Fedora | 2023.10 (분기별 날짜 기반 업데이트) | 2029년 6월 | AWS EC2 기본 |
...
| **커널** | 7.0 | 6.12 | 6.18 LTS | 6.12 LTS | 6.1 |
