...
| 인증서 회전 | cert-manager 또는 SA projected token + 자동 리로드 |
...
| 경로 | 목적 | 타임아웃 |
|------|------|---------|
| `/apis/<group>/<version>` | 리소스 목록 discovery | 5초 |
| `/openapi/v2`·`/openapi/v3` | 스키마 게시 | — |
| `/healthz`·`/readyz` | 헬스 체크 | — |
...
| `aggregator_unavailable_apiservice_total{name,reason}` | 누적 장애 카운터, `reason` 라벨 | 증가율 감시 |
...
- **감사 로그** 별도 파이프라인. kube-apiserver 감사 로그는
  extension 내부 요청 경로를 보지 못한다.
- **stale 캐시 여부** — in-memory 저장소는 재시작 시 데이터 증발.
  `startup-grace-period` 동안 readiness 미반환으로 트래픽 우회.
